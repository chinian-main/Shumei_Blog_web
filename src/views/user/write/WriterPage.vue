...
<div class="container-fluid">
  <div class="bg-light" style="height: 50px;border-radius:15px 15px 15px 15px;margin-top: 25px" >
      <Breadcrumb style="margin-left: 30px;padding-top: 10px" name="创建文章"></Breadcrumb>
    </div>
   
 
  <div class="p-2" style="background-color:#F8F9FA; border: 1px  #ccc;border-radius:20px 20px 20px 20px;margin-top: 20px">
      <Toolbar
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header text-white bg-primary">
        <h1 class="modal-title fs-5" id="exampleModalLabel">创建博客文章</h1>
            ...
            <div class="mb-3 ms-4  row ">
              ...
              <div class="mb-3  col-8 row" >
                ...
                <select class="form-select form-select-sm md-3 col-8" aria-label=".form-select-sm example" style="width:50%" v-model="artInfo.zone">
                  <option value="0" selected>分享</option>
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal" @click="add_art">保存</button>
        <button type="button" class="btn btn-primary"  @click="add_art">发布</button>
      </div>
    </div>
  </div>
</div>
    <div class="text-end">
        <button type="button" class="btn btn-primary mt-3 me-3"  data-bs-toggle="modal" data-bs-target="#Write-Modal" >创建</button>
        <button type="button" class="btn btn-secondary mt-3" data-bs-dismiss="Write-Modal">保存</button>
    </div>
 </div> 
</template>
...
export default {
  ...
  setup() {
    ...
    const add_art=()=>{
      ...
      console.log(artInfo);
      $.ajax({
        url:"http://127.0.0.1:3000/user/art/add/",
        type:"post",
        data:{
          title:artInfo.title,
        ...
        success(resp){
          if(resp.error_message==="success"){
            ...
            artInfo.description="",
            artInfo.content="",
            Modal.getInstance("#Write-Modal").hide();
